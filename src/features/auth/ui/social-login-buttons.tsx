...
import { SOCIAL_MAPS, type SocialProvider } from "../utils/social-maps";
  ...
  "flex h-[3.25rem] w-full items-center justify-center gap-[0.5rem] rounded-lg text-[1rem] font-semibold transition-all duration-200 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-50",
  {
    variants: {
      provider: {
        ...
        naver: "text-accent dark:text-accent-foreground border-none bg-[#03C75A] hover:opacity-90",
...
export function SocialButton({ provider, onClick }: SocialButtonProps) {
  ...
  return (
    <button type="button" className={socialButtonVariants({ provider })} onClick={onClick}>
      <Image src={icon} alt={label} width={20} height={20} className="h-5 w-5" />
      {label}
    </button>
  );
}
